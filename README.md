...
이 프로젝트가 실제로 Nature 논문(Farquhar et al., 2024)의 방법론을 구현한 것이라, 화면 자체를
**논문 지면**처럼 구성했습니다 (Abstract → §1 Method → §2 Results(Figure 1) → §3 Appendix).
처음엔 오실로스코프/사이버펑크 느낌(CRT 초록, 네온)으로 만들었는데, "AI 신뢰성 진단"이라는
진지한 주제와 안 어울린다는 피드백을 받아 **크림색 종이 + 세리프 폰트(Noto Serif KR / Source
Serif 4) + 딥네이비·옥스블러드 잉크 톤**으로 다시 작업했습니다.

파형 시각화 컨셉은 유지: 이 도구의 본질이 "같은 신호(질문)를 여러 번 측정했을 때 파형이
일치하는가"이기 때문입니다. 각 답변을 sine wave 하나로 표현해서:
- **같은 의미(클러스터)** → 위상이 거의 같아 서로 겹쳐 하나의 신호처럼 보임
- **다른 의미** → 위상이 어긋나 간섭무늬로 보임

결과는 "Figure 1"처럼 캡션까지 붙여서, 실제 논문의 그림-설명 관례를 그대로 따랐습니다.
